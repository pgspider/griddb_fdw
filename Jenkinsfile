...
def GRIDDB_DOCKER_PATH = '/home/jenkins/Docker/Server/Grid'
    ...
    stages {
        ...
        stage('Initialize_for_Existed_Test') {
            steps {
                catchError() {
                    sh """
                        docker exec gridserver_for_existed_test /bin/bash -c '/tmp/start_existed_test.sh ${env.GIT_BRANCH}'
                    """
                }
            }
            post {
            ...
            steps {
                catchError() {
                    sh """
                        ...
                        docker exec postgresserver_for_griddb_existed_test /bin/bash -c 'su -c "/tmp/griddb_existed_test.sh ${env.GIT_BRANCH}" postgres'
                        docker exec -w /home/postgres/postgresql-13beta2/contrib/griddb_fdw postgresserver_for_griddb_existed_test /bin/bash -c 'su -c "make clean && make && export LD_LIBRARY_PATH=/home/postgres/postgresql-13beta2/contrib/griddb_fdw/griddb/bin && make check | tee make_check.out" postgres'
                        docker cp postgresserver_for_griddb_existed_test:/home/postgres/postgresql-13beta2/contrib/griddb_fdw/make_check.out make_check_existed_test.out
                    """
                }
                script {
                    ...
                    if (status != 0) {
                        ...
                        sh 'docker cp postgresserver_for_griddb_existed_test:/home/postgres/postgresql-13beta2/contrib/griddb_fdw/regression.diffs regression.diffs'
            ...
            script {
                if (START_EXISTED_TEST == 'FAILED') {
                    if (START_ENHANCE_TEST == 'FAILED') {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Start Containers FAILED | ENHANCE_TEST: Start Containers FAILED ' + BRANCH_NAME, body: BUILD_INFO + '${BUILD_LOG, maxLines=200, escapeHtml=false}', to: "${MAIL_TO}", attachLog: false
                    } else if (INIT_ENHANCE_TEST == 'FAILED') {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Start Containers FAILED | ENHANCE_TEST: Initialize FAILED ' + BRANCH_NAME, body: BUILD_INFO + '${BUILD_LOG, maxLines=200, escapeHtml=false}', to: "${MAIL_TO}", attachLog: false
                    } else {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Start Containers FAILED | ENHANCE_TEST: Result make check ' + BRANCH_NAME, body: BUILD_INFO + '${FILE,path="make_check_enhance_test.out"}', to: "${MAIL_TO}", attachLog: false
                    }
                } else if (INIT_EXISTED_TEST == 'FAILED') {
                    if (START_ENHANCE_TEST == 'FAILED') {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Initialize FAILED | ENHANCE_TEST: Start Containers FAILED ' + BRANCH_NAME, body: BUILD_INFO + '${BUILD_LOG, maxLines=200, escapeHtml=false}', to: "${MAIL_TO}", attachLog: false
                    } else if (INIT_ENHANCE_TEST == 'FAILED') {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Initialize FAILED | ENHANCE_TEST: Initialize FAILED ' + BRANCH_NAME, body: BUILD_INFO + '${BUILD_LOG, maxLines=200, escapeHtml=false}', to: "${MAIL_TO}", attachLog: false
                    } else {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Initialize FAILED | ENHANCE_TEST: Result make check ' + BRANCH_NAME, body: BUILD_INFO + '${FILE,path="make_check_enhance_test.out"}', to: "${MAIL_TO}", attachLog: false
                    }
                } else {
                     if (START_ENHANCE_TEST == 'FAILED') {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Result make check | ENHANCE_TEST: Start Containers FAILED ' + BRANCH_NAME, body: BUILD_INFO + '${FILE,path="make_check_existed_test.out"}', to: "${MAIL_TO}", attachLog: false
                    } else if (INIT_ENHANCE_TEST == 'FAILED') {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Result make check | ENHANCE_TEST: Initialize FAILED ' + BRANCH_NAME, body: BUILD_INFO + '${FILE,path="make_check_existed_test.out"}', to: "${MAIL_TO}", attachLog: false
                    } else {
                        emailext subject: '[CI GRIDDB_FDW] EXISTED_TEST: Result make check | ENHANCE_TEST: Result make check ' + BRANCH_NAME, body: BUILD_INFO + '\n---------EXISTED_TEST---------\n' + '${FILE,path="make_check_existed_test.out"}' + '\n---------ENHANCE_TEST---------\n' + '${FILE,path="make_check_enhance_test.out"}', to: "${MAIL_TO}", attachLog: false
                    }
                }
            }
        }
