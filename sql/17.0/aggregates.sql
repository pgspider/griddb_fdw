...
--Testcase 591:
create foreign table agg_t22 (c1 text, id int) server griddb_svr;

--Testcase 592:
create foreign table agg_t23 (id int) server griddb_svr;
...
--Testcase 637:
CREATE FOREIGN TABLE btg (id int OPTIONS (rowkey 'true'), x int, y int, z text, w int) SERVER griddb_svr OPTIONS (table_name 'btg_groupby');
--Testcase 638:
INSERT INTO btg
  SELECT
    i,
...
-- pathkeys
--Testcase 646:
CREATE FOREIGN TABLE group_agg_pk (id int OPTIONS (rowkey 'true'), x int, y int, z int, w int, f int) SERVER griddb_svr OPTIONS (table_name 'group_agg_pk');
--Testcase 647:
INSERT INTO group_agg_pk
  SELECT
    i,
    i % 10 AS x,
    i % 2 AS y,
