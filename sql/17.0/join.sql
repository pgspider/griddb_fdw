...
--Testcase 798:
create foreign table tbl_ra(a int options(rowkey 'true'), b int) server griddb_svr;
--Testcase 799:
insert into tbl_ra select i, i%100 from generate_series(1,1000)i;
-- Foreign table does not support create index and analyze
...
-- This test not suitable for griddb_fdw
--   1. griddb_fdw not support JOIN pushdown
--   2. Cannot use WITH fillfactor, INDEX, VACUM, ANALYZE with foreign table.
