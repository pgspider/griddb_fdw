...
CREATE SERVER griddb_svr FOREIGN DATA WRAPPER griddb_fdw
    OPTIONS (host :GRIDDB_HOST, port :GRIDDB_PORT, clustername :CLUSTER_NAME);
...
--Testcase 1071:
CREATE FOREIGN TABLE ft_empty (id int OPTIONS (rowkey 'true'), c1 int NOT NULL, c2 text)
...
ALTER EXTENSION griddb_fdw ADD TEXT SEARCH CONFIGURATION public.custom_search;
...
-- griddb_fdw does not support JOIN pushdown
-- SEMI JOIN
...
-- Comment out these test case.
-- griddb_fdw does not need to getUserMapping in the planning phase,
-- so it is unable to check the userid is used when querying the remote table.
-- -- ====================================================================
...
-- CREATE USER MAPPING FOR regress_view_owner_another SERVER griddb_svr OPTIONS (password_required 'false');
-- EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM v4;
-- EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM (SELECT * FROM v4 UNION ALL SELECT * FROM v4);
...
-- alter server griddb_svr options (add fdw_tuple_cost '0.5');
--Testcase 288:
explain (verbose, costs off)
select array_agg(c1 order by c1 using operator(public.<^)) from ft2 where c2 = 6 and c1 < 100 group by c2;
...
--Testcase 289:
select array_agg(c1 order by c1 using operator(public.<^)) from ft2 where c2 = 6 and c1 < 100 group by c2;
-- alter server griddb_svr options (drop fdw_tuple_cost);
...
-- change (bug #17889)
--Testcase 1220:
CREATE FOREIGN TABLE loct1_rescan (id serial, c1 int) SERVER griddb_svr OPTIONS (table_name 'loct1_rescan');
--Testcase 1221:
CREATE FOREIGN TABLE loct2_rescan (id serial, c1 int, c2 text) SERVER griddb_svr OPTIONS (table_name 'loct2_rescan');

--Testcase 1222:
INSERT INTO loct1_rescan (c1) VALUES (1001);
--Testcase 1223:
INSERT INTO loct1_rescan (c1) VALUES (1002);
--Testcase 1224:
INSERT INTO loct2_rescan (c1, c2) SELECT id, to_char(id, 'FM0000') FROM generate_series(1, 1000) id;
--Testcase 1225:
INSERT INTO loct2_rescan (c1, c2) VALUES (1001, 'foo');
--Testcase 1226:
INSERT INTO loct2_rescan (c1, c2) VALUES (1002, 'bar');
--Testcase 1227:
CREATE FOREIGN TABLE remt2_rescan  (id serial OPTIONS (rowkey 'true'), c1 int, c2 text) SERVER griddb_svr OPTIONS (table_name 'loct2_rescan');
-- ANALYZE loct1_rescan;
-- ANALYZE remt2_rescan;
SET enable_mergejoin TO false;
SET enable_hashjoin TO false;
SET enable_material TO false;
-- griddb_fdw does not support RETURNING
--Testcase 1228:
EXPLAIN (VERBOSE, COSTS OFF)
...
--Testcase 1139:
CREATE TABLE parent_tbl (id int, a int, b text, c float8) PARTITION BY RANGE(a);
--Testcase 1140:
CREATE TABLE sub_parent (id int, c float8, a int, b text) PARTITION BY RANGE(a);
ALTER TABLE parent_tbl ATTACH PARTITION sub_parent FOR VALUES FROM (1) TO (10);
--Testcase 1141:
CREATE FOREIGN TABLE child_foreign (id int OPTIONS (rowkey 'true'), b text, c float8, a int)
  SERVER griddb_svr OPTIONS (table_name 'child_local');
...
--Testcase 1143:
INSERT INTO parent_tbl (id, a) VALUES(1, 1),(2, 5);
...
UPDATE rw_view SET b = 'text', c = 123.456;
--Testcase 1146:
SELECT a,b,c FROM parent_tbl ORDER BY a;
...
create foreign table tab_batch_sharded_p1 partition of tab_batch_sharded
  ...
  server griddb_svr options (table_name 'tab_batch_sharded_p1_remote');
--Testcase 1158:
...
-- GridDB required a rowkey column to insert/update
...
-- alter table rem2 drop column id;
-- copy rem2(id) from stdin;
...
\! ./griddb_restart_service_docker.sh > /dev/null
--Testcase 975:
SELECT * FROM ft1 LIMIT 10;
...
--Testcase 1031:
CREATE FOREIGN TABLE ftable ( x int OPTIONS (rowkey 'true') ) SERVER griddb_svr OPTIONS ( table_name 'batch_table', batch_size '10' );
...
--Testcase 1044:
CREATE FOREIGN TABLE ftable ( x int OPTIONS (rowkey 'true') ) SERVER griddb_svr OPTIONS ( table_name 'batch_table', batch_size '1' );
...
$$;
--Testcase 1185:
CREATE TRIGGER stmt_trig AFTER INSERT ON batch_cp_upd_test1_f
	FOR EACH STATEMENT EXECUTE FUNCTION log_stmt();
--Testcase 1186:
CREATE TRIGGER stmt_trig AFTER INSERT ON batch_cp_upd_test3_f
	FOR EACH STATEMENT EXECUTE FUNCTION log_stmt();
...
--Testcase 1187:
INSERT INTO batch_cp_upd_test VALUES (1, 2), (2, 2);
--Testcase 1188:
...
-- Because cannot create stmt_trig with remote table like postgres_fdw -> cmdlog will 0 rows
...
-- -- Test interaction of async execution with plan-time partition pruning=false
-- EXPLAIN (VERBOSE, COSTS OFF)
...
-- griddb_fdw does not support query cancel
